...
export default function PostCard({ post }: { post: any }) {
  ...
  return (
    <div className="glass-card overflow-hidden flex flex-col group hover:border-[#444] transition-all relative">
      ...
      {post.image_url ? (
        <div className="h-56 overflow-hidden bg-dark-bg border-b border-dark-border relative">
          <span className={`absolute top-4 left-4 text-[10px] font-black tracking-wider uppercase px-2 py-1 rounded shadow-lg z-10 ${statusBadgeColor}`}>
        ...
        <div className="pt-6 px-6">
          <span className={`inline-block text-[10px] font-black tracking-wider uppercase px-2 py-1 rounded shadow-lg ${statusBadgeColor} mb-2`}>
             {post.status.replace('_', ' ')}
          </span>
        </div>
      )}
      ...
      <div className={`p-6 flex flex-col flex-grow ${!post.image_url ? 'pt-2' : ''}`}>
        <div className="flex items-center justify-between mb-3 text-xs text-slate-500 font-medium font-mono tracking-tight uppercase">
          <span className="text-primary-400 font-bold tracking-wider">{post.category}</span>
          <span>{formattedDate}</span>
        </div>
        
        <h3 className="text-xl font-bold text-white mb-3 line-clamp-2 leading-snug group-hover:text-primary-400 transition-colors">
          {post.title}
        </h3>
        ...
        {post.resolution_note && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3 mb-6 text-sm">
            <strong className="text-green-500 font-bold block mb-1 uppercase text-xs tracking-wider">Resolution Note:</strong>
            <p className="text-slate-300">{post.resolution_note}</p>
          </div>
        )}
        
        <div className="mt-auto pt-4 border-t border-dark-border flex items-center justify-between text-slate-400 font-bold text-sm">
